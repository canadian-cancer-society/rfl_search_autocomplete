...
{if(!_currencyContext)_currencyContext=utils_currencyContext;_sAmount=_sAmount.replace(_currencyContext.currencySymbol,"");_sAmount=_sAmount.replace(_currencyContext.grpSepChar,"");var decSepChar=_currencyContext.decSepChar;var fracDigits=_currencyContext.fracDigits;var amount=0;var fracAmount=0;var idxDecSepChar=_sAmount.indexOf(decSepChar);if(idxDecSepChar>0&&idxDecSepChar+1<_sAmount.length){amount=parseIntStrict(_sAmount.substr(0,idxDecSepChar));var sFracAmount=_sAmount.substr(idxDecSepChar+1,fracDigits);fracAmount=parseIntStrict(sFracAmount);}
else{_sAmount=_sAmount.replace(decSepChar,"");amount=parseIntStrict(_sAmount);}
if(isNaN(amount))return NaN;if(isNaN(fracAmount))return NaN;var scalingFactor=getCurrencyScalingFactor(fracDigits);amount*=scalingFactor;if(fracAmount>0)amount+=fracAmount;return amount;}
...
var keepAliveDialog;var keepAliveTimer;function initKeepAliveDialog(){if(!keepAliveDialog){keepAliveDialog=new YAHOO.widget.SimpleDialog('keepAlive',{width:'30em',modal:true,fixedcenter:true,constraintoviewport:true,close:false});keepAliveDialog.setHeader('Session Timeout');}}
...
var buttons=[{text:'Log Out',handler:logOut},{text:'Continue Working',handler:continueWorking}];keepAliveDialog.cfg.queueProperty('buttons',buttons);keepAliveDialog.render(document.body);keepAliveDialog.show();}
...
var servletPath=_adminPath+'SessionTimeout';YAHOO.util.Cookie.set(_sessionTag,'no_session',{path:servletPath});initKeepAliveDialog();var pollExpression='keepAlivePoll("'+servletPath+'", "'+_confirmURL+'", "'+_logoutURL+'", "'+_sessionId+'")';eval(pollExpression);keepAliveTimer=setInterval(pollExpression,10000);}
function forceKeepAlive2(_confirmURL){var callback={};YAHOO.util.Connect.asyncRequest('GET',_confirmURL,callback);}
...
return true;if(keycode==13){mybutton.click();return false;}
else
return true;}
